...
md5$fastq_ftp_1 <- fastq_1
md5$fastq_ftp_2 <- fastq_2
md5 <- select(md5, fastq_ftp_1, fastq_ftp_2, fastq_md5)
...
for(i in 1:length(md5$md5_1)){
  if(grepl(pattern = "_1", x = md5$md5_1[i])){
    md5_1[i] <- md5$md5_1[i] 
  } else {
    md5_1[i] <- md5$md5_2[i]
  }
...
md5_forward <- select(md5, md5_1, fastq_ftp_1)
md5_forward$fastq_ftp_1 <- gsub("ftp.sra.ebi.ac.uk\\/vol1\\/fastq\\/[SERR]{2,3}[0-9]{2,3}\\/[SERR]{2,3}[0-9]{3,7}\\/", 
                                "", md5_forward$fastq_ftp_1)
md5_forward$fastq_ftp_1 <- gsub("ftp.sra.ebi.ac.uk\\/vol1\\/fastq\\/[SERR]{2,3}[0-9]{2,3}\\/[0-9]{2,3}\\/[SERR]{2,3}[0-9]{3,7}\\/",
                                "", md5_forward$fastq_ftp_1)
colnames(md5_forward) <- c('md5sum', "file")


md5_reverse <- select(md5, md5_2, fastq_ftp_2)
md5_reverse$fastq_ftp_2 <- gsub("ftp.sra.ebi.ac.uk\\/vol1\\/fastq\\/[SERR]{2,3}[0-9]{2,3}\\/[SERR]{2,3}[0-9]{3,7}\\/", 
                                "", md5_reverse$fastq_ftp_2)
md5_reverse$fastq_ftp_2 <- gsub("ftp.sra.ebi.ac.uk\\/vol1\\/fastq\\/[SERR]{2,3}[0-9]{2,3}\\/[0-9]{2,3}\\/[SERR]{2,3}[0-9]{3,7}\\/",
                                "", md5_reverse$fastq_ftp_2)
...
write_tsv(x = md5, path = "inputs/md5checksums.txt", col_names = F)
