...
knitr::opts_chunk$set(echo = TRUE, fig.path = "Rmd_figures/", cache = T, message = F)
```
...
```{r}
# Inputs needed:
# outputs/optimal_rf/*acc.csv
# outputs/optimal_rf/*_optimal_rf.RDS
# outputs/vita_rf/*_vita_vars.txt
...
# read_csv("sandbox/41pangenomes/41pangenome-hash-to-gather-match.csv")
# ~/Desktop/at_least_5_studies_vita_vars_gather_all_lca.csv
# "sandbox/41genome_sigs/41-hash-to-gather-match.csv"
```
...
acc <- unlist(snakemake@input[['acc']]) %>%
  map_dfr(read_csv) 
...
refseq <- unlist(snakemake@input[['gather_refseq']]) %>%
  set_names() %>% 
...
all <- unlist(snakemake@input[['gather_all_db']]) %>%
  set_names() %>% 
  map_dfr(read_csv, .id = "source") %>%
  mutate(source = gsub("outputs\\/gather\\/", "", source)) %>%
...
genbank <- unlist(snakemake@input[['gather_genbank']]) %>%
  set_names() %>% 
  map_dfr(read_csv, .id = "source") %>%
  mutate(source = gsub("outputs\\/gather\\/", "", source)) %>%
...
nbhd_queries <- unlist(snakemake@input[['sgc_pangenome_gather_all_db']]) %>%
  set_names() %>% 
  map_dfr(read_csv, .id = "source") %>%
...
# predictive hash
hash_to_genome <- read_csv(snakemake@input[['hash_to_gather_map_loso']]) %>%
  select(-X1) %>%
...
lca <- read_csv(snakemake@input[['lca']])
...
hash_to_pangenome <- read_csv(snakemake@input[['hash_to_gather_map_pangenome']]) %>%
  select(-X1) %>%
...
```{r}
tmp <- read_csv(snakemake@input[['gather_pangenome_vita']])
...
print(paste0("The pangenomes alone capture ", 
             round(sum(pangenome_importance$norm, na.rm =T) / 6 * 100, digits = 3),
